...
#define MPU6050												//定义我们使用的传感器为MPU6050 Define the sensor we use as MPU6050
#define MOTION_DRIVER_TARGET_MSP430		//定义驱动部分,采用MSP430的驱动(移植到STM32F4) Define the driver part, using the MSP430 driver (ported to STM32F4)
...
//}
#define log_i 	printf	//打印信息 Print information
#define log_e  	printf	//打印信息 Print information
...
//q30格式,long转float时的除数. q30 format, divisor when converting long to float.
#define q30  1073741824.0f

//陀螺仪方向设置 Gyroscope direction setting
...
//空函数,未用到. Empty function, not used.
...
//    printf("res = %d\r\n",res);
	...
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置所需要的传感器 Set up the required sensors
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//设置FIFO Setting up FIFO
		if(res)return 2; 
		...
		if(res)return 5; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//设置dmp功能 Setting the dmp function
