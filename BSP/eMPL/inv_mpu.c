...
#define MPU6050												//��������ʹ�õĴ�����ΪMPU6050 Define the sensor we use as MPU6050
#define MOTION_DRIVER_TARGET_MSP430		//������������,����MSP430������(��ֲ��STM32F4) Define the driver part, using the MSP430 driver (ported to STM32F4)
...
//}
#define log_i(...) 	// Disabled
#define log_e(...) 	// Disabled
...
//q30��ʽ,longתfloatʱ�ĳ���. q30 format, divisor when converting long to float.
#define q30  1073741824.0f

//�����Ƿ������� Gyroscope direction setting
...
u8 mpu_dmp_init(void)
{
    ...
    res = mpu_init();
//    printf("res = %d\r\n",res);
	if(res==0)	//��ʼ��MPU6050  Initialize MPU6050
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//��������Ҫ�Ĵ����� Set up the required sensors
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//����FIFO Setting up FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//���ò����� Setting the Sample Rate
		if(res)return 3; 
		res=dmp_load_motion_driver_firmware();		//����dmp�̼� Load dmp firmware
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//���������Ƿ��� Set gyroscope orientation
		if(res)return 5; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//����dmp���� Setting the dmp function
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//����DMP�������(��󲻳���200Hz) Set the DMP output rate (maximum 200Hz)
		if(res)return 7;           
//		res=run_self_test();		//�Լ� Self-Test
//		if(res)return 8;           
		res=mpu_set_dmp_state(1);	//ʹ��DMP Enabling DMP
		if(res)return 9;     
	}
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	...
	{
		q0 = quat[0] / q30;	//q30��ʽת��Ϊ������ Convert q30 format to floating point number
		q1 = quat[1] / q30;
		q2 = quat[2] / q30;
		q3 = quat[3] / q30; 
		//����õ�������/�����/�����
